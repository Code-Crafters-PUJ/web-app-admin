...
export class ManageAccountComponent implements OnInit {
  ...
  postEntity(): void {
    if(this.permiso)
      {
    if (this.url === "new") {
      if (this.verificarCampos() && this.esCorreoValido(this.credential.email)) {
        const formData = {
          ...
          response => {
            if (response.message == "El email ya esta registrado") {
              ...
              if (this.isAdmin) {
                ...
                this.router.navigate(['/home/Monitoreo/accounts']);
              }
            }
          },
          error => {
  ...
  guardarCambios(): void {
    ...
    const nuevaInformacion = this.obtenerNuevaInformacionDesdeCheckbox()
    ...
    const id = parseInt(this.accountId, 10);
    if (this.esCorreoValido(datosActualizados.email) && datosActualizados.email) {
        ...
        response => {
          if (response.message == "El correo electrónico ya está en uso") {
            Swal.fire({
              ...
              text: "El correo ya existe en otra cuenta",
              icon: 'warning',
              confirmButtonText: 'OK'
            });
          }
          else if (response.message == "El número de identificación ya está en uso por otra cuenta") {
            Swal.fire({
              title: 'Modificación Fallida!',
              text: "El ID ya esta presente en otra cuenta",
              icon: 'warning',
              confirmButtonText: 'OK'
            });
          }
